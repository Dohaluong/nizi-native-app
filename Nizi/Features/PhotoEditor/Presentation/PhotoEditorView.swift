...
///
/// Sprint 2 (Editor foundation) scope only: load a preview, Cancel/Save, press-and-hold to view
/// the original, unsaved-change detection. No Preset/Adjust/Auto tools yet (Bước 4–6) — this view
/// intentionally has no tool tray below the image yet, so as not to build UI for features that
/// don't exist.
struct PhotoEditorView: View {
    ...
    init(
        ...
        imageLoader: PhotoEditorImageLoading = PhotoEditorAssetLoader(),
        repository: PhotoEditRepository = InMemoryPhotoEditRepository(),
        onClose: @escaping (PhotoEditorResult) -> Void
    ) {
        self.onClose = onClose
        _viewModel = State(initialValue: PhotoEditorViewModel(context: context, imageLoader: imageLoader, repository: repository))
    }
        ...
        case .loaded:
            if let image = viewModel.displayedImage {
                ...
                .onLongPressGesture(minimumDuration: 0, maximumDistance: .infinity) {
                    ...
                    viewModel.isShowingOriginal = isPressing
                }
            }
        }
    }
